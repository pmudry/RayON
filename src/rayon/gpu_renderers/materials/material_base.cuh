 ...
 */
__device__ __forceinline__ f3 reflect(const f3 &v, const f3 &n) { return v - 2.0f * dot(v, n) * n; }
 ...
 */
template <typename Derived> struct MaterialBase
{
   /**
    * @brief Get reference to derived material (CRTP downcast)
    */
   __device__ __forceinline__ const Derived &derived() const { return static_cast<const Derived &>(*this); }

   __device__ __forceinline__ Derived &derived() { return static_cast<Derived &>(*this); }

   // Shared utilities accessible to all derived materials
   // (These delegate to the free functions above for better optimization)

   __device__ __forceinline__ f3 do_reflect(const f3 &v, const f3 &n) const { return reflect(v, n); }

   __device__ __forceinline__ f3 do_refract(const f3 &uv, const f3 &n, float eta) const { return refract(uv, n, eta); }
