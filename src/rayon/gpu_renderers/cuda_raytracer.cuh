...
__device__ inline f3 rotate_vector(f3 v, f3 angles)
{
   ...
   if (angles.x != 0.0f)
   {
      float c = cosf(angles.x), s = sinf(angles.x);
      f3 t = v;
      v.y = t.y * c - t.z * s;
      v.z = t.y * s + t.z * c;
   }
   if (angles.y != 0.0f)
   {
      float c = cosf(angles.y), s = sinf(angles.y);
      f3 t = v;
      v.x = t.x * c + t.z * s;
      v.z = -t.x * s + t.z * c;
   }
   if (angles.z != 0.0f)
   {
      float c = cosf(angles.z), s = sinf(angles.z);
      f3 t = v;
      v.x = t.x * c - t.y * s;
      v.y = t.x * s + t.y * c;
   }
...
__device__ inline bool hit_mesh_triangle(const CudaScene::MeshTriangle &tri, const ray_simple &r, float t_min,
                                         float t_max, hit_record_simple &rec)
{
   return hit_triangle(tri.v0, tri.v1, tri.v2, tri.n0, tri.n1, tri.n2, tri.smooth_shadings, r, t_min, t_max, rec);
}
                                ...
                                const ray_simple &r_world, float t_min, float t_max, hit_record_simple &rec)
{
   ...
   // 3. Scale: Scale the ray origin and direction by the inverse of the mesh scale
   //    (Note: If you scale a model by 2x, the ray moves "half as fast" through it locally)
   ...
   if (hit_anything)
   {  
      // Transform intersection point and normal back to world space, because 't' is invariant under scaling assuming uniform scaling
      rec.p = r_world.at(rec.t); // Use t directly (valid because we scaled direction)
      // Un-scale: Normals scale by 1/scale factor
      f3 n_local = f3(rec.normal.x / scale.x, rec.normal.y / scale.y, rec.normal.z / scale.z);
      // Un-rotate: Apply the forward rotation
      rec.normal = normalize(rotate_vector(n_local, rotation));
      // Ensure normal points towards the ray (standard front-facing check)
      if (rec.front_face)
         rec.normal = dot(r_world.dir, rec.normal) < 0 ? rec.normal : -rec.normal;
      else
         rec.normal = dot(r_world.dir, rec.normal) < 0 ? rec.normal : -rec.normal;
   }
   return hit_anything;
...
__device__ __forceinline__ bool intersect_geometry(const CudaScene::Scene &scene, const CudaScene::Geometry &geom,
                                                   ...
                                                   hit_record_simple &rec)
{
   ...
   case GeometryType::TRIANGLE:
      return hit_triangle(geom.data.triangle.v0, geom.data.triangle.v1, geom.data.triangle.v2, geom.data.triangle.n0,
                          geom.data.triangle.n1, geom.data.triangle.n2, geom.data.triangle.smooth_shadings, r, t_min, t_max,
                          rec);
   ...
   case GeometryType::TRIANGLE_MESH:
      ...
      {
         return hit_mesh(scene.meshes[geom.data.mesh_instance.mesh_id], geom.data.mesh_instance.translation,
                         geom.data.mesh_instance.rotation, geom.data.mesh_instance.scale, r, t_min, t_max, rec);
      }
      return false;
   default:
      return false;
