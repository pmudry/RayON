...
private:
    ...
    bool hit(const ray &r, interval ray_t, hit_record &rec) const override {
        ...
        vec3 normal(0, 0, 0);
        double eps = 1e-6;
        if (std::abs(local_hit.x() - x_interval.min) < eps) normal = vec3(-1, 0, 0);
        else if (std::abs(local_hit.x() - x_interval.max) < eps) normal = vec3(1, 0, 0);
        else if (std::abs(local_hit.y() - y_interval.min) < eps) normal = vec3(0, -1, 0);
        else if (std::abs(local_hit.y() - y_interval.max) < eps) normal = vec3(0, 1, 0);
        else if (std::abs(local_hit.z() - z_interval.min) < eps) normal = vec3(0, 0, -1);
        else if (std::abs(local_hit.z() - z_interval.max) < eps) normal = vec3(0, 0, 1);
