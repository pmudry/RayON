                                ...
                                float cam_v_x, float cam_v_y, float cam_v_z)
{
   ...
   float3_simple cam_u(cam_u_x, cam_u_y, cam_u_z);
   float3_simple cam_v(cam_v_x, cam_v_y, cam_v_z);
   ...
   for (int s = 0; s < samples_to_add; s++)
   {
      ...
      float3_simple ray_origin = camera_center;
      if (g_dof_enabled && g_dof_aperture > 0.0f)
      {
         // Find focus point along original ray direction
         float3_simple normalized_dir = unit_vector(ray_direction);
         float3_simple focus_point = camera_center + g_dof_focus_distance * normalized_dir;
         
         // Offset ray origin randomly on aperture disk
         float3_simple aperture_offset = sample_aperture_disk(cam_u, cam_v, local_rand_state);
