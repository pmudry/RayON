...
__device__ __forceinline__ float3_simple operator*(float t, const float3_simple &v)
{
   return v * t;
}

/** @brief Compute dot product of two vectors */
__device__ __forceinline__ float dot(const float3_simple &a, const float3_simple &b)
{
   return a.x * b.x + a.y * b.y + a.z * b.z;
}

/** @brief Compute cross product of two vectors */
__device__ __forceinline__ float3_simple cross(const float3_simple &a, const float3_simple &b)
{
   return float3_simple(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
}

/** @brief Normalize a vector to unit length */
__device__ __forceinline__ float3_simple unit_vector(const float3_simple &v)
{
   return v / v.length();
}

/** @brief Convert a normal to a debug RGB color */
__device__ __forceinline__ float3_simple normal_to_color(const float3_simple &n)
{
   return float3_simple(0.5f * (n.x + 1.0f), 0.5f * (n.y + 1.0f), 0.5f * (n.z + 1.0f));
}

/** @brief Create grayscale color from single value */
__device__ __forceinline__ float3_simple grayscale(float v)
{
