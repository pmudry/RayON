...
public:
    bool parseFile(const string& filename) {
        ...
        if (!file.is_open()) {
            cerr << "ERROR: Cannot open scene file: " << filename << endl;
            return false;
        }
...
static bool loadMaterials(const SimpleYAMLParser& parser, SceneDescription& scene,
    ...
    for (int i = 0; i < 100; ++i) {  // Reasonable limit
        ...
        cout << "  Loaded material: " << mat_name << " (ID: " << mat_id << ")" << endl;
    }
...
static bool loadGeometry(const SimpleYAMLParser& parser, SceneDescription& scene,
    ...
    for (int i = 0; i < 1000; ++i) {  // Reasonable limit
        ...
        if (it == material_name_to_id.end()) {
            cerr << "ERROR: Unknown material: " << mat_name << endl;
            continue;
        }
        ...
        cout << "  Loaded " << geom_type << " with material " << mat_name << endl;
    }
...
bool loadSceneFromYAML(const char* filename, SceneDescription& scene) {
    ...
    if (!parser.parseFile(filename)) {
        cerr << "ERROR: Failed to parse YAML file" << endl;
        return false;
    }
...
bool saveSceneToYAML(const char* filename, const SceneDescription& scene) {
    ...
    for (size_t i = 0; i < scene.materials.size(); ++i) {
        ...
        file << "    type: ";
        switch (mat.type) {
            case MaterialType::LAMBERTIAN: file << "\"lambertian\""; break;
            case MaterialType::METAL: file << "\"metal\""; break;
            case MaterialType::MIRROR: file << "\"mirror\""; break;
            case MaterialType::ROUGH_MIRROR: file << "\"rough_mirror\""; break;
            case MaterialType::GLASS: file << "\"glass\""; break;
            case MaterialType::LIGHT: file << "\"light\""; break;
            default: file << "\"lambertian\""; break;
        }
        file << "\n";
        ...
        if (mat.pattern != ProceduralPattern::NONE) {
            ...
            file << "      type: ";
            switch (mat.pattern) {
                case ProceduralPattern::FIBONACCI_DOTS: file << "\"fibonacci_dots\""; break;
                case ProceduralPattern::CHECKERBOARD: file << "\"checkerboard\""; break;
                case ProceduralPattern::STRIPES: file << "\"stripes\""; break;
                default: break;
            }
            file << "\n";
            file << "      color: [" << mat.pattern_color.x() << ", " << mat.pattern_color.y() << ", " << mat.pattern_color.z() << "]\n";
            if (mat.pattern_param1 > 0) file << "      dot_count: " << (int)mat.pattern_param1 << "\n";
            if (mat.pattern_param2 > 0) file << "      dot_radius: " << mat.pattern_param2 << "\n";
        }
    }
    ...
    for (const auto& geom : scene.geometries) {
        ...
        switch (geom.type) {
            case GeometryType::SPHERE:
                ...
                file << "    center: [" << geom.data.sphere.center.x() << ", " 
                     << geom.data.sphere.center.y() << ", " << geom.data.sphere.center.z() << "]\n";
            ...
            case GeometryType::RECTANGLE:
                file << "\"rectangle\"\n";
                file << "    material: \"material_" << geom.material_id << "\"\n";
                file << "    corner: [" << geom.data.rectangle.corner.x() << ", " 
                     << geom.data.rectangle.corner.y() << ", " << geom.data.rectangle.corner.z() << "]\n";
                file << "    u: [" << geom.data.rectangle.u.x() << ", " 
                     << geom.data.rectangle.u.y() << ", " << geom.data.rectangle.u.z() << "]\n";
                file << "    v: [" << geom.data.rectangle.v.x() << ", " 
                     << geom.data.rectangle.v.y() << ", " << geom.data.rectangle.v.z() << "]\n";
                break;
    ...
    file.close();
    cout << "Scene saved to: " << filename << endl;
    return true;
}
