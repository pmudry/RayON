 ...
 */
inline double sdSphere(const Vec3 &p, const Vec3 &center, double radius) { return (p - center).length() - radius; }
 ...
 * @return Signed distance
 */
inline double sdPlane(const Vec3 &p, const Vec3 &normal, double distance) { return dot(p, normal) + distance; }
...
inline double sdDeathStar(const Vec3 &p, const Vec3 &center, double ra, double rb, double d)
{
   ...
   {
      return std::max(p2.length() - ra, -(p2 - Vec3(d, 0.0, 0.0)).length() + rb);
   }
}
 ...
 */
inline double opUnion(double d1, double d2) { return std::min(d1, d2); }

/**
 * @brief Subtraction (d1 - d2)
 */
inline double opSubtraction(double d1, double d2) { return std::max(d1, -d2); }

/**
 * @brief Intersection of two SDFs
 */
inline double opIntersection(double d1, double d2) { return std::max(d1, d2); }
...
{
   return Vec3(p.x() - period.x() * std::round(p.x() / period.x()), p.y() - period.y() * std::round(p.y() / period.y()),
 ...
 */
template <typename DistFunc> inline Vec3 estimateNormal(const Point3 &p, DistFunc dist_func, double epsilon = 0.001)
{
   // Central differences for gradient
   Vec3 grad(dist_func(p + Vec3(epsilon, 0, 0)) - dist_func(p - Vec3(epsilon, 0, 0)),
...
inline RayMarchResult rayMarch(const Ray &r, DistFunc dist_func, Interval ray_t,
                               const RayMarchConfig &config = RayMarchConfig())
{
