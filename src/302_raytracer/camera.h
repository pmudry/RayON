...
class Camera
{
 ...
 public:
   ...
   bool renderTiled(vector<unsigned char> &image, int tiles_x, int tiles_y, SDL_Renderer *renderer,
                    SDL_Texture *texture, SDL_Texture *logo_texture, const SDL_Rect &logo_rect, void *font,
      ...
      for (int tile_y = 0; tile_y < tiles_y; tile_y++)
      {
         for (int tile_x = 0; tile_x < tiles_x; tile_x++)
         {
            ...
            unsigned long long tile_ray_count = ::renderPixelsCUDA(
                tile_buffer.data(), actual_tile_width, actual_tile_height, camera_center.x(), camera_center.y(),
                camera_center.z(), tile_pixel00.x(), tile_pixel00.y(), tile_pixel00.z(), pixel_delta_u.x(),
   ...
   void drawUIControls(SDL_Renderer *renderer, void *font_ptr, float gamma, float light_intensity,
                       float background_intensity, bool accumulation_enabled, SliderBounds *gamma_slider,
                       SliderBounds *intensity_slider, SliderBounds *background_slider, SDL_Rect *toggle_button_rect)
   {
#ifdef SDL2_TTF_FOUND
      TTF_Font *font = static_cast<TTF_Font *>(font_ptr);
      if (!font)
      ...
      if (!small_font)
         small_font = font; // Fallback to regular font
                                  ...
                                  bool auto_accumulate = true)
   {
      ...
      unsigned char *logo_data = stbi_load("../res/ISC Logo inline white v3 - 1500px.png", &logo_img_width,
                                           &logo_img_height, &logo_img_channels, 4); // Force RGBA
      ...
      while (running)
      {
            ...
            else if (event.type == SDL_MOUSEBUTTONDOWN)
            {
               if (event.button.button == SDL_BUTTON_LEFT)
               {
                  ...
                  // Check if clicking on gamma slider
                           ...
                           my >= gamma_slider_bounds.y - 5 &&
                           my <= gamma_slider_bounds.y + gamma_slider_bounds.height + 5)
                  {
                     ...
                     gamma = gamma_slider_bounds.min_val +
                             ratio * (gamma_slider_bounds.max_val - gamma_slider_bounds.min_val);
                     gamma = std::max(gamma_slider_bounds.min_val, std::min(gamma_slider_bounds.max_val, gamma));
                     needs_rerender = true;
                  }
                  // Check if clicking on intensity slider
                  else if (mx >= intensity_slider_bounds.x &&
                           mx <= intensity_slider_bounds.x + intensity_slider_bounds.width &&
                           my >= intensity_slider_bounds.y - 5 &&
                           my <= intensity_slider_bounds.y + intensity_slider_bounds.height + 5)
                  {
                     dragging_slider = true;
                     ...
                     light_intensity = intensity_slider_bounds.min_val +
                                       ratio * (intensity_slider_bounds.max_val - intensity_slider_bounds.min_val);
                     light_intensity = std::max(intensity_slider_bounds.min_val,
                                                std::min(intensity_slider_bounds.max_val, light_intensity));
                     ::setLightIntensity(light_intensity);
                     camera_changed = true; // Restart rendering
                  }
                  // Check if clicking on background slider
                  else if (mx >= background_slider_bounds.x &&
                           mx <= background_slider_bounds.x + background_slider_bounds.width &&
                           my >= background_slider_bounds.y - 5 &&
                           my <= background_slider_bounds.y + background_slider_bounds.height + 5)
                  {
                     dragging_slider = true;
                     ...
                     background_intensity =
                         background_slider_bounds.min_val +
                         ratio * (background_slider_bounds.max_val - background_slider_bounds.min_val);
                     background_intensity = std::max(background_slider_bounds.min_val,
                                                     std::min(background_slider_bounds.max_val, background_intensity));
                     ::setBackgroundIntensity(background_intensity);
                     camera_changed = true; // Restart rendering
                  }
                  else
         ...
         if (needs_rerender && current_samples > 0)
         {
            ...
            drawUIControls(renderer, font, gamma, light_intensity, background_intensity, accumulation_enabled,
                           &gamma_slider_bounds, &intensity_slider_bounds, &background_slider_bounds,
                           &toggle_button_rect);
         ...
         if (current_samples < max_samples && !camera_changed && running && accumulation_enabled)
         {
            ...
            unsigned long long cuda_ray_count = ::renderPixelsCUDAAccumulative(
                display_image.data(), accum_buffer.data(), image_width, image_height, camera_center.x(),
                camera_center.y(), camera_center.z(), pixel00_loc.x(), pixel00_loc.y(), pixel00_loc.z(),
                pixel_delta_u.x(), pixel_delta_u.y(), pixel_delta_u.z(), pixel_delta_v.x(), pixel_delta_v.y(),
                pixel_delta_v.z(), actual_samples_to_add, current_samples, max_depth,
